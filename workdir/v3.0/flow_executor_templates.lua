--[[
  wrk2 flow executor for template-driven v2.1 iterations.
...
local debug_non2xx = (os.getenv("FLOW_DEBUG_NON2XX") or "0") == "1"
local debug_non2xx_body_max_bytes = tonumber(os.getenv("FLOW_DEBUG_NON2XX_BODY_MAX_BYTES") or "4096") or 4096
if debug_non2xx_body_max_bytes < 0 then
  debug_non2xx_body_max_bytes = 0
end
...
local function init_runtime_counters()
  _G.v21_requests_issued = 0
  _G.v21_pairs = 0
  _G.v21_responses_2xx = 0
  _G.v21_responses_non2xx = 0
  _G.v21_iterations_started = 0
  _G.v21_iterations_completed = 0
  _G.v21_iterations_failed = 0
  _G.v21_iterations_exhausted = 0
  _G.v21_template_resolve_failures = 0
  _G.v21_decode_failures = 0
  _G.v21_encode_failures = 0
  _G.v21_stage = stage
  _G.v21_data_dir = data_dir

  _G.v21_step_keys_line = ""
  _G.v21_step_status_keys_line = ""
  _G.v21_non2xx_debug_count = 0
  _G.v21_debug_non2xx_enabled = debug_non2xx and 1 or 0
end

local function register_step_key(step_key)
  local token = sanitize_token(step_key)
  local key_var = "v21_step_key_name__" .. token
  if _G[key_var] == nil then
    _G[key_var] = step_key
    local prior = _G.v21_step_keys_line
    if not prior or prior == "" then
      _G.v21_step_keys_line = token
    else
      _G.v21_step_keys_line = prior .. "," .. token
    end
  end
  if _G["v21_step_count__" .. token] == nil then
    _G["v21_step_count__" .. token] = 0
    _G["v21_step_sum_us__" .. token] = 0
    _G["v21_step_max_us__" .. token] = 0
  end
  return token
end
...
local function register_step_status_key(step_key, status)
  ...
  _G["v21_step_status_name__" .. token] = step_key .. ":" .. tostring(status)
  _G["v21_step_status__" .. token] = _G["v21_step_status__" .. token] or 0
  local prior = _G.v21_step_status_keys_line
  if not prior or prior == "" then
    _G.v21_step_status_keys_line = token
  else
    _G.v21_step_status_keys_line = prior .. "," .. token
  end
  return token
end
...
local function record_step_metrics(step_key, status, elapsed_us)
  ...
  local count_name = "v21_step_count__" .. token
  local sum_name = "v21_step_sum_us__" .. token
  local max_name = "v21_step_max_us__" .. token
  ...
  local scount_name = "v21_step_status__" .. stoken
  _G[scount_name] = (_G[scount_name] or 0) + 1
end

local function record_non2xx_debug(st, status, body, conn_id)
  if not debug_non2xx then
    return
  end

  local body_text = tostring(body or "")
  local body_truncated = false
  if #body_text > debug_non2xx_body_max_bytes then
    body_text = string.sub(body_text, 1, debug_non2xx_body_max_bytes)
    body_truncated = true
  end

  local line = nil
  line = select(1, encode_json({
    ts_us = now_us(),
    executor = "flow_executor_templates",
    thread_id = thread_id,
    conn_id = conn_id,
    iteration_index = st.current_step and st.current_step.iteration_index or nil,
    step_index = st.current_step and st.current_step.step_index or nil,
    flow_id = st.current_step and st.current_step.flow_id or nil,
    method = st.current_step and st.current_step.method or nil,
    path = st.current_step and st.current_step.path or nil,
  ...
  local idx = (_G.v21_non2xx_debug_count or 0) + 1
  _G.v21_non2xx_debug_count = idx
  _G["v21_non2xx_debug_line__" .. tostring(idx)] = line
end
...
local function claim_next_iteration(st)
  ...
  if total == 0 then
    st.exhausted = true
    _G.v21_iterations_exhausted = _G.v21_iterations_exhausted + 1
    return false
  end
  ...
  _G.v21_iterations_started = _G.v21_iterations_started + 1
  return true
end

local function fail_iteration(st)
  _G.v21_iterations_failed = _G.v21_iterations_failed + 1
...
local function complete_iteration(st)
  _G.v21_iterations_completed = _G.v21_iterations_completed + 1
  st.iteration_index = nil
  st.step_index = 1
  st.current_step = nil
  st.current_path_params = nil
...
local function build_request_for_state(st)
  ...
  if path_err then
    _G.v21_template_resolve_failures = _G.v21_template_resolve_failures + 1
    fail_iteration(st)
    return nil
  end
  ...
  if req_body ~= nil and method ~= "GET" and method ~= "HEAD" then
    local encoded = nil
    encoded, _ = encode_json(req_body)
    if not encoded then
      _G.v21_encode_failures = _G.v21_encode_failures + 1
      fail_iteration(st)
      return nil
    end
    body_text = encoded
  ...
  _G.v21_requests_issued = _G.v21_requests_issued + 1
...
function response(status, headers, body, conn_id)
  ...
  if is_success_status(status) then
    _G.v21_responses_2xx = _G.v21_responses_2xx + 1
    ...
    if body and body ~= "" then
      ...
      if decoded ~= nil then
        parsed = decoded
      elseif derr then
        _G.v21_decode_failures = _G.v21_decode_failures + 1
      end
    end
    ...
    _G.v21_responses_non2xx = _G.v21_responses_non2xx + 1
    record_non2xx_debug(st, status, body, conn_id)
  ...
  if log_every > 0 and (_G.v21_pairs % log_every == 0) then
    print(string.format(
...
local function flow_try_write_non2xx_debug(lines)
  if not lines or #lines == 0 then
    return
  end
  local dir = os.getenv("FLOW_STATS_OUT_DIR")
  if not dir or dir == "" then
    print("FLOW_DEBUG_NON2XX: FLOW_STATS_OUT_DIR is empty, skipping non2xx debug output")
    return
  end

  local name = string.format(
    "non2xx_debug_%s_%s.jsonl",
    os.date("!%Y%m%dT%H%M%S"),
    string.sub(string.gsub(tostring(os.clock()), "%.", ""), 1, 12)
  )
  local path = dir .. "/" .. name
  local f = io.open(path, "w")
  if not f then
    print("FLOW_DEBUG_NON2XX: cannot open " .. path)
    return
  end
  ...
  f:close()
  print(string.format("non2xx_debug_file: %s records=%d", path, #lines))
end
...
function done(summary, latency, requests)
  ...
  for _, t in ipairs(threads) do
    out.requests_issued = out.requests_issued + (t:get("v21_requests_issued") or 0)
    out.pairs = out.pairs + (t:get("v21_pairs") or 0)
    out.responses_2xx = out.responses_2xx + (t:get("v21_responses_2xx") or 0)
    out.responses_non2xx = out.responses_non2xx + (t:get("v21_responses_non2xx") or 0)
    out.iterations_started = out.iterations_started + (t:get("v21_iterations_started") or 0)
    out.iterations_completed = out.iterations_completed + (t:get("v21_iterations_completed") or 0)
    out.iterations_failed = out.iterations_failed + (t:get("v21_iterations_failed") or 0)
    out.iterations_exhausted = out.iterations_exhausted + (t:get("v21_iterations_exhausted") or 0)
    out.template_resolve_failures = out.template_resolve_failures + (t:get("v21_template_resolve_failures") or 0)
    out.decode_failures = out.decode_failures + (t:get("v21_decode_failures") or 0)
    out.encode_failures = out.encode_failures + (t:get("v21_encode_failures") or 0)
    if not stage_name then
      stage_name = t:get("v21_stage")
    end
    if not data_root then
      data_root = t:get("v21_data_dir")
    end

    for _, token in ipairs(split_csv(t:get("v21_step_keys_line"))) do
      if not step_token_seen[token] then
        step_token_seen[token] = true
        step_tokens[#step_tokens + 1] = token
      end
    end
    for _, token in ipairs(split_csv(t:get("v21_step_status_keys_line"))) do
  ...
  for _, token in ipairs(step_tokens) do
    ...
    local step_name = nil
    for _, t in ipairs(threads) do
      count = count + (t:get("v21_step_count__" .. token) or 0)
      sum_us = sum_us + (t:get("v21_step_sum_us__" .. token) or 0)
      local th_max = t:get("v21_step_max_us__" .. token) or 0
      if th_max > max_us then
        max_us = th_max
      end
      if not step_name then
        step_name = t:get("v21_step_key_name__" .. token)
      end
    end
    local avg_us = 0
    if count > 0 then
  ...
  for _, token in ipairs(status_tokens) do
    ...
    for _, t in ipairs(threads) do
      total = total + (t:get("v21_step_status__" .. token) or 0)
      if not status_name then
        status_name = t:get("v21_step_status_name__" .. token)
      end
    end
  ...
  local non2xx_debug_lines = {}
  for _, t in ipairs(threads) do
    local count = t:get("v21_non2xx_debug_count") or 0
    for i = 1, count do
      local line = t:get("v21_non2xx_debug_line__" .. tostring(i))
      if line and line ~= "" then
        non2xx_debug_lines[#non2xx_debug_lines + 1] = line
      end
    end
  end
  ...
  flow_try_write_non2xx_debug(non2xx_debug_lines)
end
